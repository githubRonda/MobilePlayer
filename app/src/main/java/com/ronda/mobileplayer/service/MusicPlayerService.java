package com.ronda.mobileplayer.service;
...
public class MusicPlayerService extends Service {
    ...
    private void openAudio(int position) {
        ...
        if (mediaItems != null && mediaItems.size() > 0) {
            ...
            // 模仿VideoView#openVideo()的源码, 在切换视频时, reset(), 再调用 release()方法, 最后再创建一个MediaPlayer
            if (mediaPlayer != null) {
//                mediaPlayer.release();
//                mediaPlayer.reset();
                mediaPlayer.reset();
            ...
            try {
                ...
                mediaPlayer.prepareAsync();// 网络音乐的时候必须要用prepareAsync()而不是prepare(), 但prepareAsync()也会走prepare()途径
    ...
    class  MyOnCompletionListener implements MediaPlayer.OnCompletionListener {
        ...
        public void onCompletion(MediaPlayer mp) {
            // 播放完成的回调, 继续播放下一首. (当设置setLooping(true)时, 该方法不会被触发)
            next();
        }
    }
...
//            notifyChange(OPENAUDIO);
            EventBus.getDefault().post(mediaItem); // 使用EventBus 代替
            start();
        }
    }
    ...
    private void start() {
        ...
        intent.putExtra("notification",true);//标识来自状态拦.  因为点击通知时,需要跳转至播放界面, 并且继续播放
